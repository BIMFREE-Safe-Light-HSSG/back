...
async def read_viewer_buildings(
...
@router.get("/bootstrap", response_model=ViewerBootstrapResponse)
async def read_viewer_bootstrap(
    current_user: Annotated[dict[str, Any], Depends(get_current_user)],
) -> ViewerBootstrapResponse:
    bootstrap = await get_viewer_bootstrap(current_user)
    return ViewerBootstrapResponse(**bootstrap)
