...
- `GET /auth/me`: access token으로 현재 사용자 조회
- `POST /data_transform/upload`: 변환 task 생성 및 MinIO presigned PUT URL 발급, 인증 필요
- `POST /data_transform/{task_id}/complete_upload`: 업로드 완료 처리, 모델 서버 변환 요청, `graph_data` 저장, 인증 필요
- `GET /dev/users`: 개발 편의용 가입 유저 조회, 인증 필요
- `GET /health`: 서버 상태 확인
...
성공 시 access token과 사용자 정보를 반환한다. 프론트엔드는 보호된 API 호출 시
`Authorization: Bearer <access_token>` 형태로 사용할 수 있다.
